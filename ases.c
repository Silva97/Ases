...
int  fsize(FILE* file);
void filter(char* dest, char* source);
...
int main(int argc, char* argv[]){
	...
	filter(code, source); // Remove any non-instruction character
	
	// If insert or remove instruction, edit the "ac" string in the filter() function
	while(code[ip] != 0){
		switch(code[ip]){
			case '@':                      break;
